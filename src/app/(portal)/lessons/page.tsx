...
export default async function LessonsPage() {
  ...
  const { data: lessons } = await supabase
    .from("lessons")
    .select("id,course_name,tutor_name,grade_background,description,price_cents,session_type,availability,profiles(full_name,email,phone,user_roles(roles(name)))")
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_380px]">
        ...
        {canCreateLesson ? (
          ...
          <EmptyState title="Permission required" description={noCreatePermissionMessage("lesson")} />
        )}
      </div>
    </>
  );
